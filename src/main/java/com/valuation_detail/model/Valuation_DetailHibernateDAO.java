...
public class Valuation_DetailHibernateDAO implements Valuation_Detail_Interface {
	...
	public List Select_valuation_detailALL(String vlt_id) throws Exception {
		...
		try {
			...
			Query query = session.createQuery(GET_ONE_STMT_VALUAYIONID);
	...
	public void delete(String vlt_id, String prod_id) throws Exception {
		...
		try {
			session.beginTransaction();
			Query query = session.createQuery(DELETE_VALUATION_DETAIL);
			query.setString(0, vlt_id);
			query.setParameter(1, prod_id);
			query.executeUpdate();
			session.getTransaction().commit();
		} catch (RuntimeException ex) {
